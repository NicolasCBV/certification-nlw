...
import com.rocketseat.certificationlw.modules.students.entities.AnswersCertificationsEntity;
import com.rocketseat.certificationlw.modules.students.entities.CertificationStudentEntity;
import com.rocketseat.certificationlw.modules.students.entities.StudentEntity;
import com.rocketseat.certificationlw.modules.students.repositories.CertificationStudentRepo;
import com.rocketseat.certificationlw.modules.students.repositories.StudentRepo;
...
public class StudentCertificationAnswersUseCase {
	...
	public CertificationStudentEntity exec(StudentCertificationAnswerDTO dto) throws Exception {
		var hasCertification =
			this.verifyIfHasCertificationUseCase.exec(new VerifyIfHasCertificationDTO(dto.getEmail(), dto.getTechnology()));

		if(hasCertification) throw new Exception("Você já tirou sua certificação");

		List<QuestionEntity> questionsEntity = questionRepo.findByTechnology(dto.getTechnology());
		...
		var student = this.studentRepo.findByEmail(dto.getEmail());
		UUID studentID;
		if(student.isEmpty()) {
			var studentCreated = StudentEntity.builder().email(dto.getEmail()).build();
			studentCreated = this.studentRepo.save(studentCreated);
		...
		var certificationStudentCreated = this.certificationStudentRepo.save(certificationStudentEntity);
		...
		certificationStudentRepo.save(certificationStudentEntity);
